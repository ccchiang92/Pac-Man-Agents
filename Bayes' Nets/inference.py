...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        beliefCopy=util.Counter()
        for p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos, prob in newPosDist.items():
                beliefCopy[newPos]+=self.getBeliefDistribution()[p]*prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        if len(self.legalPositions)<=(self.numParticles):
            for i in range(self.numParticles):
                ...
                self.particleList+=[self.legalPositions[index]]
        else:
            j=0
            for position in self.legalPositions:
                if (j%interval)==0:
                    self.particleList+=[position]
                j+=1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None:
            ...
            for i in range(self.numParticles):
                trueDistance = util.manhattanDistance(pacmanPosition,self.particleList[i])
                distribution[self.particleList[i]]+=emissionModel[trueDistance]
            if distribution.totalCount()==0:
                self.initializeUniformly(gameState)
            else:
                self.particleList=[]
                for h in range(self.numParticles):
                    self.particleList+=[util.sample(distribution)]
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            particleListCopy+=[util.sample(newPosDist)]
        self.particleList=particleListCopy
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        possibleLocations = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(possibleLocations)
        self.particleList=[]
        interval=len(possibleLocations)/(self.numParticles)
        if len(possibleLocations)<=(self.numParticles):
            for i in range(self.numParticles):
                index=i%len(possibleLocations)
                self.particleList+=[possibleLocations[index]]
        else:
            j=0
            for position in possibleLocations:
                if (j%interval)==0:
                    self.particleList+=[position]
                j+=1
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particleTuple in self.particleList:
            for k in range(self.numGhosts):
                if JailedGhost[k]:
                    particleTuple=self.getParticleWithGhostInJail(particleTuple, k)
            temp=1
            for x in range(self.numGhosts):
                if not JailedGhost[x]:
                    trueDistance = util.manhattanDistance(pacmanPosition,particleTuple[x])
                    temp=temp*emissionModels[x][trueDistance]
            distribution[particleTuple]+=temp
        if distribution.totalCount()==0:
            self.initializeParticles()
        else:
            self.particleList=[]
