...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        CompareDistance= float("inf")

        for beliefs in livingGhostPositionDistributions:
            if CompareDistance>self.distancer.getDistance(pacmanPosition,beliefs.argMax()):
                ClosestGhost=beliefs.argMax()
        returnAction=legal[0]
        compareDist= self.distancer.getDistance(ClosestGhost, Actions.getSuccessor(pacmanPosition, legal[0]))
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            temp = self.distancer.getDistance(ClosestGhost, successorPosition)
            if temp<compareDist:
                returnAction=action
                compareDist=temp
